    ...
    */
    then(onFulfilled, onRejected){
        ...
        return new Promise((resolve, reject) => {
                ...
                self.onFulfilledCallbacks.push(() => {
                    try{
                        ...
                        setTimeout(() => {
                            ...
                            // 分两种情况：
                            // 1. 回调函数返回值是Promise，执行then操作，一旦成功了就resolve(value)，失败了reject(reason)
            ...
            }else if(self.status === FULFILLED){  // 当前Promise已经成功了，执行回调函数
                try{
                    setTimeout(() => {
                        const result = onFulfilled(self.value);
                        result instanceof Promise ? result.then(resolve, reject) : resolve(result);
   ...
   */
   ...
   static race(promises){
       return new Promise((resolve, reject) => {
           promises.forEach( p => {
               Promise.resolve(p).then(
                   value => resolve(value),  // 某个成功则立即调用resolve(value)
                   reason => reject(reason)  // 某个失败则立即调用reject(reason) 
               )
           })
       })
   }
...
Promise.prototype.myAllSetted = function(promises){
    return new Promise((resolve, reject) => {
        let n = promises.length;
        let count = 0;
        let res = new Array(n);
